...
function initTypewriter() {
  ...
  function tick() {
    ...
    el.style.textShadow = `0 0 40px ${word.color}66`;

    if (!deleting) {
      charIdx++;
      el.textContent = word.text.slice(0, charIdx);
      ...
      if (charIdx === 0) {
        ...
        el.style.color = TYPEWRITER_WORDS[wordIdx].color;
        setTimeout(tick, 300);
        return;
      }
...
async function loadContent() {
  try {
    ...
    function setHTML(id, key) {
      const el = document.getElementById(id);
      // 只有值不為空才更新，避免空字串覆蓋 HTML 預設內容
      if (el && map[key] && map[key].trim() !== '') {
        el.innerHTML = map[key].replace(/\\n/g, '<br>').replace(/\n/g, '<br>');
      }
    }

    // ⚠️ Hero 主標題 (txt-hero-t1, txt-hero-t2, typewriter-text) 完全不從 DB 載入
    // 副標題允許 DB 覆蓋
